...
\begin{document}
\title{
  A view of $0$ or $1$ elements: \tcode{views::nullable}

  And a concept to constrain \tcode{maybe}s
}
\author{
...
\begin{abstract}
  ...
  It also proposes a concept \tcode{maybe}, that types which are contextually convertible to bool and dereferenceable model.
\end{abstract}
...
The \tcode{nullable_view} adapts types that model \tcode{maybe}. In addition to the adapter, this paper propsoses exposing the concept and the most generally used helper functions for \tcode{maybe}s
\begin{itemize}
\item \tcode{value_or}---The referred to value or an alternative
\item \tcode{reference_or}---A reference to the referred to value or a reference to an alternative
\item \tcode{or_invoke}---The referred to value or a function to invoke to return a value
\item \tcode{yield_if}---An optional holding a value or not conditioned on a boolean
\end{itemize}

These mirror functions commonly found in optional implementations, however most of the implementations were specified before traits like \tcode{common_reference} were standardised, and can not be fixed today. For example \tcode{std::optional<T>::value_or(U)} returns a T.

The concept is very minimal, as most uses of a \tcode{maybe} impose few constraints.
...
concept maybe = requires(const T t) {
  ...
  \end{minted}
\caption{concept maybe}
\end{minipage}
\end{figure}

This ensures that a user type provides a conversion to \tcode{bool} and a derefence operator that can be used for \tcode{const} qualified types.
...
The type \tcode{nullable_view} naturally meets the requirements for freestanding, as do the concept and proposed functions.
...
\begin{codeblock}
namespace std {
  // \ref{maybe.maybe}, class template \tcode{maybe}
template <class T>
concept maybe = requires(const T t) {
    bool(t);
    *(t);
...
\pnum
\mandates
\tcode{T} models \tcode{maybe}
\tcode{common_type_t<iter_reference_t<T>, U\&\&>::type} is defined

\pnum
\returns
\tcode{bool(m) ? static_cast<R>(*m) : static_cast<R>(forward<U>(u))}.
\end{itemdescr}
...
\mandates
\tcode{T} models \tcode{maybe}
\tcode{common_reference<iter_reference_t<T>, U\&\&>::type} is defined

\pnum
\constraints
\tcode{!reference_constructs_from_temporary_v<R, U>}
\tcode{!reference_constructs_from_temporary_v<R, T\&>}

\pnum
\returns
\tcode{bool(m) ? static_cast<R>(*m) : static_cast<R>((U\&\&)u)}
\end{itemdescr}
...
\mandates
\tcode{T} models \tcode{maybe}
\tcode{common_type<iter_reference_t<T>, invoke_result_t<I>>::type} is defined

\pnum
\returns
\tcode{bool(m) ? static_cast<R>(*m) : static_cast<R>(invocable())}
\end{itemdescr}
