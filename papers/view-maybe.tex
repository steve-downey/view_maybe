...
In writing range transformation it is useful to be able to lift a value into a view that is either empty or contains the value. For types that are \tcode{nullable}, constructing an empty view for disengaged values and providing a view to the underlying value is useful as well. The adapter \tcode{views::single} fills a similar purpose for non-nullable values, lifting a single value into a view, and \tcode{views::empty} provides a range of no values of a given type. The type \tcode{views::maybe} can be used to unify \tcode{single} and \tcode{empty} into a single type for further processing. This is in particuluar useful when translating list comprehensions.
...
\Sec3[range.maybe.nullable]{Concept nullable}
...
\pnum{}
For convience, the exposition-only concepts \exposconcept{is-reference-wrapper-v}. \exposconcept{nullable}, \exposconcept{nullable_ref}, and \exposconcept{copyable_object} are used below.
...
\begin{itemdescr}
  \pnum{}
  \effects{}
  Initializes value_ with \tcode{std::move(maybe)}.
\end{itemdescr}
...
\begin{itemdescr}
  \pnum{}
  \effects{}
  Initializes value\_ with \tcode{std::move(maybe)}.
\end{itemdescr}
...
\begin{itemdescr}
  \pnum{}
  \effects{}
  Initializes value\_ as if by \tcode{value_{in_place, forward<Args>(args)...}}.
\end{itemdescr}
