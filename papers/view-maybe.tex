...
\chapter{Design}
For \tcode{maybe_view} the design is a hybrid of view_empty and view_single, with the straight forward extension for reference type, referring to an existing object. For \tcode{nullable_view} we have the same sematics of zero or one objects, only based on if the underlying nullable object does or does not have a value.
